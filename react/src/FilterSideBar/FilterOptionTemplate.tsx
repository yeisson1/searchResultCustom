...
const useHasScrolled = (elementRef:any) => {
...
const FilterOptionTemplate = ({
  ...
  const handleClear = useCallback(
    () =>
      navigateToFacet(
        filters.filter((filter:any) => filter.selected),
        preventRouteChange
      ),
      ...
      <>
        ...
        {shouldTruncate && (
          <ShowMoreFilterButton
            ...
            truncated={truncated}
            toggleTruncate={() => openTruncated((isTruncated:any) => !isTruncated)}
          />
        )}
      </>
    )
      ...
      <div className={titleContainerClassName}>
        ...
        {appliedFiltersOverview === 'show' && filters && !selected && (
          <div className={classNames(handles.filterSelectedFilters, 'f6')}>
            {filters
              .filter((facet:any) => facet.selected)
