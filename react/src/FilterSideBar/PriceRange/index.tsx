...
const PriceRange = ({ title, facets, priceRange, priceRangeLayout, scrollToTop }:any) => {
  const [range, setRange] = useState()
  const { culture, setQuery } = useRuntime()
  const intl = useIntl()
  const navigateTimeoutId = useRef(null)
  ...
  const handleChange = ([left, right]:any) => {
  ...
  const availableOptions = facets.filter(({ slug }:any) => slugRegex.test(slug))
  ...
  availableOptions.forEach(({ slug }:any) => {
    const [, minSlug, maxSlug] = slug.match(slugRegex)

    const min = parseInt(minSlug, 10)
    const max = parseInt(Math.ceil(maxSlug) as any, 10)
    ...
    >
      {priceRangeLayout === 'inputAndSlider' && (
        <PriceRangeInput
          ...
          onSubmit={(newRange:any) => setRange(newRange)}
          max={maxValue}
          min={minValue}
        />
      )}
      <Slider
        ...
        defaultValues={defaultValues}
        formatValue={(value:any) => formatCurrency({ intl, culture, value })}
        values={range}
        range
      />
    </FilterOptionTemplate>
