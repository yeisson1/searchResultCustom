...
const FilterSidebar = ({
  ...
  const isFilterSelected = (slectableFilters: any, filter:any) => {
    return slectableFilters.find(
      (filterOperation:any) => filter.value === filterOperation.value
    )
  }

  const handleFilterCheck = (filter:any) => {
  ...
  const handleClearFilters = (key:any = "") => {
  ...
  const handleUpdateCategories = (maybeCategories:any) => {
  ...
  const context = useMemo(() => {
    const { query, map }:any = filterContext
...
const updateTree = (categories:any) =>
  produce((draft) => {
    ...
    while (
      !(
        currentLevel.find(
          (category:any) => category.value === categories[0].value
        ) || currentLevel.every((category:any) => !category.selected)
      )
    ) {
      currentLevel = currentLevel.find((category:any) => category.selected).children
    }

    categories.forEach((category:any) => {
      const selectedIndex = currentLevel.findIndex(
        (cat:any) => cat.value === category.value
      )
...
// from the API.
const useCategoryTree = (initialTree:any, categoryTreeOperations:any) => {
