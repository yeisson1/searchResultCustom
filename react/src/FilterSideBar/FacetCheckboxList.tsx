...
const FacetCheckboxList = ({
  ...
  const filteredFacets = useMemo(() => {
    ...
    return facets.filter(
      (facet:any) => facet.name.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1
    )
  ...
  return (
    <>
      ...
      {filteredFacets.slice(0, endSlice).map((facet:any) => {
        const { name } = facet
      ...
      {shouldTruncate && (
        <ShowMoreFilterButton
          ...
          truncated={truncated}
          toggleTruncate={() =>
            openTruncated((prevTruncated:any) => !prevTruncated)
          }
        />
      )}
    </>
