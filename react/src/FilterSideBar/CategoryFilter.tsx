...
const getSelectedCategories = (rootCategory:any) => {
  let node = rootCategory.children
  const selectedCategories = [rootCategory]

  while (node) {
    const category = node.find((categ:any) => categ.selected)
...
const CategoryFilter = ({
  ...
  const { map  } = useFilterNavigator() as any
  const handles = useCssHandles(CSS_HANDLES)

  const selectedCategories = getSelectedCategories(category)

  const handleUnselectCategories = (index:any) => {
          ...
          lastSelectedCategory.children.length > 0 && (
            ...
            >
              <Collapsible
                ...
                render={(childCategory:any, index:number) => (
                  <CategoryItem
