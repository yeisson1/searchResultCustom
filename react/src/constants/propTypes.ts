...
import { LAYOUT_MODE } from '../FilterSideBar/LayoutModeSwitcher'
...
export const searchResultPropTypes = {
  ...
  mobileLayout: PropTypes.shape({
    /** First Layout Mode */
    mode1: PropTypes.oneOf(LAYOUT_MODE.map((mode:any) => mode.value)),
    /** Second Layout Mode */
    mode2: PropTypes.oneOf(LAYOUT_MODE.map((mode:any) => mode.value)),
  }),
}
